  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button id="received_create" type="button" class="btn btn-default">OK</button>
	...
	<script>
	                ...
	                { "targets": 10,
						...
						"render": function ( data, type, full, meta ) {
							...
							if(type === 'export'){
		                     ...
		                     return '<select class="select_status_col '+select_class+'" id="select_status_col'+full.id+'" onchange="change_status(this,'+full.id+',this.value)"><option value=1 class=success '+status1+'>הוזמן</option><option value=2 class=error '+status2+'>בוטל</option><option value=3 class=label-primary '+status3+'>התקבל</option><option value=4 class=success '+status4+'>נשלח</option></select>';
	    ...
	    function change_status(select,id,val){
			...
			if(val == 1){
				...
				$(select).removeClass('success');
				$(select).removeClass('error');
				receivedBox(id,formType,'התקבל');
			}else if(val == 4){
				$(select).removeClass('label-primary');
				$(select).addClass('success');
				$(select).removeClass('error');
				receivedBox(id,formType,'נשלח');
			}
	    ...
	    function receivedBox(formId,formType,actionType){
	    	//reset fields
	    	...
	    	$('#received_create').attr('data-actionType', actionType);
